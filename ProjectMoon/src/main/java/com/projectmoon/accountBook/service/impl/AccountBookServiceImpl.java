package com.projectmoon.accountBook.service.impl;
...
public class AccountBookServiceImpl implements AccountBookService{
	...
	public List<AccountBookVO> SelectSettle(AccountBookVO accountBookVO) {
		...
		// 1. 은행 목록 가져오기
		CdCommonVO cdCommonVO = new CdCommonVO();
		cdCommonVO.setCode("BANK");
		List<CdCommonVO> bankList = cdCommonDao.selectCode(cdCommonVO);
		
		// 2. 전월잔액
		for(int i = 0; i < bankList.size(); i++) {
			cdCommonVO = bankList.get(i);
			String bank = cdCommonVO.getMainCategory();
			prevVO.setBank(bank);
